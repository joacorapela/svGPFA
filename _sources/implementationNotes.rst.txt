...
The estimation of svGPFA parameters is performed using the Expectation Conditional Maximisation algorithm (:cite:t:`mcLachlanAndKrishnan08`, see :meth:`svGPFA.stats.svEM.SVEM.maximize`), which reduces to a sequence of numerical optimisations. Because we use PyTorch's autograd to compute derivatives, these optimisations only require the calculation of the svGPFA lower bound (left hand side of Eq. 4 in :cite:t:`dunckerAndSahani18`). Below we provide details about how the calculation of this lower bound is implemented.
