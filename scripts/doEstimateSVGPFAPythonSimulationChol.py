...
def main(argv):
    ...
    nTestPoints = int(estMetaDataConfig["control_variables"]["nTestPoints"])
    randomEmbedding = estMetaDataConfig["control_variables"]["randomEmbedding"].lower()=="true"
    indPointsLocsKMSRegEpsilon = float(estMetaDataConfig["control_variables"]["indPointsLocsKMSRegEpsilon"])
    ...
    if randomEmbedding:
        ...
        CFilename = estMetaDataConfig["embedding_params"]["C_filename"]
        dFilename = estMetaDataConfig["embedding_params"]["d_filename"]
        C0, d0 = utils.svGPFA.configUtils.getLinearEmbeddingParams(CFilename=CFilename, dFilename=dFilename)
    ...
    if estMetaDataConfig["variational_params"]["qMuDist"]=="Uniform":
        ...
        indPointsMeans = utils.svGPFA.initUtils.getZeroIndPointsMeans(nTrials=nTrials, nLatents=nLatents, nIndPointsPerLatent=nIndPointsPerLatent)
    else:
        raise ValueError("Only qMuDist=Uniform is currently supported")
    ...
    estimResConfig["estimation_params"] = {"estMetaDataFilename": estimationMetaDataFilename}
