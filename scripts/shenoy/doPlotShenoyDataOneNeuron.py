...
def main(argv):
    ...
    fig = plot.svGPFA.plotUtilsPlotly.getSpikesTimesPlotOneNeuron(
        spikes_times=spikes_times, neuron_index=neuron_index, title=title, xlabel=xlabel)
    png_fig_filename = fig_filename_pattern.format(neuron_index, location, "png")
    fig.write_image(png_fig_filename)
    html_fig_filename = fig_filename_pattern.format(neuron_index, location, "html")
