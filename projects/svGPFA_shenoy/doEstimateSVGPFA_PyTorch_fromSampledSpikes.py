...
def main(argv):
    ...
    parser.add_argument("spikes_model_number", help="number of the model from which spikes were samples", type=int)
    ...
    parser.add_argument("--data_filename_pattern", help="data filename pattern",
                        default="results/{:08d}_spikesFromSampledCIF.pickle")
    args = parser.parse_args()

    estInitNumber = args.estInitNumber
    spikes_model_number = args.spikes_model_number
    ...
    data_filename = data_filename_pattern.format(spikes_model_number)
    with open(data_filename, "rb") as f:
        loadRes = pickle.load(f)
    spikesTimes = loadRes["spikesTimes"]
    neuron_indices = [i for i in range(len(spikesTimes[0]))]
...
#                                         min_nSpikes_perNeuron_perTrial)
    ...
    neurons_indices = [i for i in range(nNeurons)]
